...
while 1:
    ...
    while 1:
        ...
        # function to Check for new messages
            ...
            for line in file:
                # check if there are any '1's in their message file (1 indicates a new message)
                if (line.split("\t")[2].strip() == "1"):
                    # check if that user with a new message doesn't already exist in outgoing message
                    if (line.split("\t")[1].strip() not in outgoing):
                        # add users with new messages to outgoing message
                        outgoing += line.split("\t")[1].strip() + "\n"
            file.close()
            connectionSocket.send(outgoing.encode())
            ...
            # populate filedata with data from file
            file = open(filename, 'r')
            filedata = file.read()
            file.close

            # replace all 0's from that user to 1's (indicating the message has been seen
            newdata = filedata.replace(sender + "\t" + "1", sender + "\t" + "0")
            file = open(filename, 'w')
            file.write(newdata)
            file.close()

            # add messages from that particular user onto the outgoing  message
            file = open(filename, 'r')
            outgoing = ""
            for line in file:
                if (line.split("\t")[1].strip().upper() == sender.upper()):
